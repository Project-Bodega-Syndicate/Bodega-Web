...
const ProductDetails = () => {
  const { id } = useParams();
  const { singlePrd, setSinglePrd } = useAppContext();
        ...
        const fetchProductDetail = () => {
          const fetchProductDetail = async () => {
            try {
              ...
              const response = await axios.get(`${prdURL}${id}/`, {
                headers: headers,
              });
              if (response) {
  ...
  }, [singlePrd, id, prdURL, setSinglePrd]);
      ...
      {isLoading ? (
        ...
        <>
          <div className="w-full">
            <img className="w-full" alt="" src={singlePrd.product_image1}></img>
          </div>
          ...
          <button className="p-2 mt-10 h-10 bg-sky-600 rounded-lg text-center">
            Visit Creator's Profile
          </button>
          <div className="flex flex-row justify-between w-4/5 mt-6 pb-12">
            <button className="p-2 h-10 bg-neutral-800 rounded-lg text-center border-solid border-2 border-white w-36">
              Yerrr
            </button>
            <button className="p-2 h-10 bg-neutral-800 rounded-lg text-center border-solid border-2 border-white w-36">
              Buy
